<div>
    <style>
        /* styles.css */
        .tree ul {
            list-style-type: none;
        }

        .tree ul,
        .tree li {
            margin: 0;
            padding: 0;
        }

        .tree .caret {
            cursor: pointer;
            user-select: none;
        }

        .tree .caret::before {
            content: "\25B6";
            /* Triangle pointing right */
            color: black;
            display: inline-block;
            margin-right: 6px;
        }

        .tree .caret-down::before {
            transform: rotate(90deg);
            /* Triangle pointing down */
        }

        .tree .nested {
            display: none;
        }

        .tree .active {
            display: block;
        }

        li.item:hover {
            background-color: blue;
            /* color: blue; */
        }
    </style>
    {{-- Close your eyes. Count to one. That is how long forever feels. --}}
    @if (session()->has('message'))
    <div class="alert alert-danger alert-dismissible fade show m-2" role="alert">

        {{ session('message') }}
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>
    @endif
    <!-- Debugging -->
    <div class="m-2">
        <label for="school_id">Ecoles:</label>
        {{-- <select id="school_id" wire:model='school_id' name="school_id" class="form-select"> --}}
            {{-- <option value="">Sélectionner une ecole</option> --}}
            <div class="tree border p-2">
                <ul>

                    <li>
                        <span class="caret"><i class="fa fa-folder">{{$libelle}}</i>

                            <ul class="nested form-control">
                                @foreach($schools as $school)
                                <li class="item"><button class="btn btn-lick" wire:model='cursus_id'
                                        wire:click='test({{ $school->id }})'>{{
                                        $school->school }}_{{
                                            $school->city }}
                                    </button></li>
                                @endforeach
                            </ul>
                        </span>
                    </li>

                </ul>
            </div>
            {{-- <option value="{{ $school->id }}"> {{ $school->school }}</option> --}}

            {{--
        </select> --}}
        @error('school_id') <span class="text-danger">*{{ $message }}</span> @enderror
    </div>

    <div class="m-2">
        <label for="school_id">Cursus:</label>
        {{-- <select id="school_id" wire:model='school_id' name="school_id" class="form-select"> --}}
            {{-- <option value="">Sélectionner une ecole</option> --}}
            <div class="tree border p-2">
                <ul>

                    <li>
                        <span class="caret"><i class="fa fa-folder">{{$libelleCursus}}</i>
                            <ul class="nested form-control">
                                @foreach($cursuss as $cursus)
                                <li class="item"><button class="btn btn-lick" wire:model='cursus_id'
                                        wire:click='testLevel({{ $cursus->id }})'>{{
                                        $cursus->name }}
                                    </button></li>
                                @endforeach
                                <li class="item"><button class="btn btn-lick" wire:model='cursus_id'>
                                  selectioner un cursus
                                </li>
                            </ul>
                        </span>
                    </li>

                </ul>
            </div>
            {{-- <option value="{{ $school->id }}"> {{ $school->school }}</option> --}}

            {{--
        </select> --}}
        @error('curriculum_id') <span class="text-danger">*{{ $message }}</span> @enderror
    </div>
    <form wire:submit.prevent="submit">
        @csrf
        @if ($levelname)
        <div class="m-2">
            <label for="title" class="form-label">Niveau:</label>
            <input type="text" id="title" value="{{$levelname->name}}" readonly class="form-control">
        </div>
        @endif
        <div class="m-2">
            <label for="title" class="form-label">Nom de la filiere:</label>
            <input type="text" id="title" wire:model="name" name="name" class="form-control">
            @error('name') <span class="text-danger">*{{ $message }}</span> @enderror
        </div>

        <div class="m-2">
            <label for="cursus_id">Nouveau niveau:</label>
            <select id="level_id" wire:model="level_id" name="level_id" class="form-select">
                <option value="">Sélectionner un niveau</option>
                @foreach($levels as $level)
                <option value="{{ $level->id }}">{{ $level->name }}</option>
                @endforeach
            </select>
            @error('level_id') <span class="text-danger">*{{ $message }}</span> @enderror
        </div>
        <div class="m-2">
            <button type="submit" class="form-control btn btn-primary">Enregistrer</button>
        </div>
    </form>
</div>
<script>
    document.addEventListener('DOMContentLoaded', function () {
        var toggler = document.getElementsByClassName("caret");
        for (var i = 0; i < toggler.length; i++) {
            toggler[i].addEventListener("click", function () {
                this.parentElement.querySelector(".nested").classList.toggle("active");
                this.classList.toggle("caret-down");
            });
        }
    });
</script>
